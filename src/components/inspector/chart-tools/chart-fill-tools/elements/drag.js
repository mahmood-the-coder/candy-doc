...
export function initInspectorChartFillColorsDrag() {
  ...
  window.addEventListener("mousedown", (e) => {
    if (
      e.target.classList.contains(
        "candyDoc__inspectorChartFillColorDragIcon"
      )
    ) {
    ...
    swap(
      ...
      [
        ...container.querySelectorAll(
          ".candyDoc__inspectorChartFillColorDragItem"
        ),
  ...
  window.addEventListener("mouseup", () => {
    ...
    if (!options) return;
    options.fill.colors = children.map((c) => c.value);
