...
export const rowColors = document.createElement("div");
rowColors.classList.add("candyDoc__inspectorChartGridColors");
...
colorsLabel.innerText = "Row Color(s)";
...
addIcon.addEventListener("mousedown", () => {
  ...
  if (!options) return;
  const lastColor = options.grid.row.colors[options.grid.row.colors.length - 1];
  options.grid.row.colors.push(lastColor);
  selected.dataset.chartOptions = JSON.stringify(options);
  renderApexChart(options)
  addChartRowColors(options.grid.row.colors, true);
});
export function addChartRowColors(colorsArray) {
  ...
  for (let index = 0; index < colorsArray.length; index++) {
    ...
    input.addEventListener("input", (e) => {
      ...
      const options = JSON.parse(selected?.dataset?.chartOptions ?? "null");
      if (!options) return;
      options.grid.row.colors[e.target.parentElement.dataset.index] = e.target.value;
      selected.dataset.chartOptions = JSON.stringify(options);
      renderApexChart(options)
    });
    ...
    colorItemClone
      ...
      .addEventListener("mousedown", (e) => {
        ...
        const options = JSON.parse(selected?.dataset?.chartOptions ?? "null");
        if (!options) return;
        if (options.grid.row.colors.length <= 1) return;
        options.grid.row.colors.splice(parseInt(index), 1);
        selected.dataset.chartOptions = JSON.stringify(options);
        renderApexChart(options)

        e.target.parentElement?.remove();
        if (options.grid.row.colors.length <= 1) {
