...
export const colors = document.createElement("div");
colors.classList.add("candyDoc__inspectorChartOptionsColors","colors");
const colorItem = document.createElement("div");
colorItem.classList.add(
  "candyDoc__inspectorChartOptionsColorsItem",
  "candyDoc__inspectorChartOptionsColorDragItem"
);
...
colorsLabel.innerText = "Color(s)";
colorsLabel.classList.add("candyDoc__inspectorChartOptionsColorsLabel");
colorItem.type = "color";
colors.dataset.chartType = "line area bar rangeBar rangeArea heatmap treemap donut radialBar radar";
...
dragIcon.classList.add(
  "candyDoc__icon",
  "candyDoc__inspectorChartColorDragIcon",
  "dragIcon"
);
dragIcon.innerHTML =
  /*html*/
...
addIcon.addEventListener("mousedown", () => {
  ...
  if (!options) return;
  const lastColor = options.colors[options.colors.length - 1];
  options.colors.push(lastColor);
  selected.dataset.chartOptions = JSON.stringify(options);
  renderApexChart(options)
  addChartOptionsColors(options.colors, true);
});
export function addChartOptionsColors(colorsArray, isDistributed) {
  
  colors.innerHTML = "";
  colors.append(colorsLabel);
  if (isDistributed ) {colors.append(addIcon)}
  
  for (let index = 0; index < colorsArray.length; index++) {
    if (!isDistributed && index == 0) {
      ...
      input.addEventListener("input", (e) => {
        ...
        const options = JSON.parse(selected?.dataset?.chartOptions ?? "null");
        if (!options) return;
        options.colors[e.target.parentElement.dataset.index] = e.target.value;
      ...
      colorItemClone.dataset.index = 0;
      colors.append(colorItemClone);
      colorItemClone.querySelector(".removeIcon").remove();
      colorItemClone.querySelector(".dragIcon").remove();
      break;
    }
    const color = colorsArray[index];
    const colorItemClone = colorItem.cloneNode(true);
    ...
    colorItemClone
      ...
      .addEventListener("mousedown", (e) => {
        ...
        options.colors.splice(parseInt(index), 1);
        selected.dataset.chartOptions = JSON.stringify(options);
        renderApexChart(options)
