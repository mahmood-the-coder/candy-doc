...
export function initColBorderColorsDrag(container) {
  ...
  window.addEventListener("mouseup", () => {
    ...
    if(!selected)return;
    [...container.querySelectorAll(".candyDoc__inspectorTableColBorderColors")].forEach((item,index)=>{
      item.dataset.index=index.toString()
    })
    const colors=[...container.querySelectorAll(".candyDoc__inspectorTableColBorderColors")].map(i=>i.dataset.col_borderColor);
