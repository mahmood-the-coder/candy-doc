...
export const colBorderColors = document.createElement("div");
colBorderColors.classList.add("candyDoc__inspectorTableColBorderColors");
const colorItem = document.createElement("div");
colorItem.classList.add(
  "candyDoc__inspectorTableColBorderColorItem",
...
addIcon.addEventListener("mousedown", () => {
  ...
  const colors = JSON.parse(
    selected?.dataset?.col_borderColors ?? "[]"
  );
  colors.push("#ffffff");
  selected.dataset.col_borderColors=JSON.stringify(colors)
  addTableColsBorderColors(colors)
  applyColColors(selected)
});
export function addTableColsBorderColors(colorsArray) {
  colBorderColors.innerHTML = "";
  colBorderColors.append(colorsLabel,addIcon);
  ...
  for (let index = 0; index < colorsArray.length; index++) {
    ...
    colBorderColors.append(colorItemClone);
    const color = colorsArray[index];
    colorItemClone.dataset.col_borderColor = color;
    ...
    input.addEventListener("input", (e) => {
      const selected = getSelected();
      if (!selected) return;
      const colBorderColors = JSON.parse(
        selected?.dataset?.col_borderColors ?? "[]"
      );
      colBorderColors[input.parentElement.dataset.index] =
        e.target.value;
      selected.dataset.col_borderColors =
        JSON.stringify(colBorderColors);
      input.style.color = e.target.value;
      input.style.backgroundColor = e.target.value;
      input.parentElement.dataset.col_borderColor = e.target.value;
      applyColColors(selected);
    });
...
window.addEventListener("mousedown",(e)=>{
    if(!e.target.classList.contains("candyDoc__inspectorTableColBorderColorItemRemoveIcon"))return;
    const index = e.target.parentElement.dataset.index;

    const selected = getSelected();
    if (!selected) return;
 
    const colBorderColorArray = JSON.parse(
      selected?.dataset?.col_borderColors ?? "[]"
    );
   
    if (colBorderColorArray.length == 0){
     
      return;
    }
    colBorderColorArray.splice(parseInt(index), 1);
    selected.dataset.col_borderColors = JSON.stringify(
      colBorderColorArray
    );
