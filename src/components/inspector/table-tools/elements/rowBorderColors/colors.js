...
export const rowBorderColors = document.createElement("div");
rowBorderColors.classList.add("candyDoc__inspectorTableRowBorderColors");
const colorItem = document.createElement("div");
colorItem.classList.add(
  "candyDoc__inspectorTableRowBorderColorItem",
...
addIcon.addEventListener("mousedown", () => {
  ...
  const colors = JSON.parse(
    selected?.dataset?.row_borderColors ?? "[]"
  );
  colors.push("#ffffff");
  selected.dataset.row_borderColors=JSON.stringify(colors)
  addTableRowsBorderColors(colors)
  applyRowColors(selected)
});
export function addTableRowsBorderColors(colorsArray) {
  rowBorderColors.innerHTML = "";
  rowBorderColors.append(colorsLabel,addIcon);
  ...
  for (let index = 0; index < colorsArray.length; index++) {
    ...
    rowBorderColors.append(colorItemClone);
    const color = colorsArray[index];
    colorItemClone.dataset.row_borderColor = color;
    ...
    input.addEventListener("input", (e) => {
      const selected = getSelected();
      if (!selected) return;
      const rowBorderColors = JSON.parse(
        selected?.dataset?.row_borderColors ?? "[]"
      );
      rowBorderColors[input.parentElement.dataset.index] =
        e.target.value;
      selected.dataset.row_borderColors =
        JSON.stringify(rowBorderColors);
      input.style.color = e.target.value;
      input.style.backgroundColor = e.target.value;
      input.parentElement.dataset.row_borderColor = e.target.value;
      applyRowColors(selected);
    });
...
window.addEventListener("mousedown",(e)=>{
    if(!e.target.classList.contains("candyDoc__inspectorTableRowBorderColorItemRemoveIcon"))return;
    const index = e.target.parentElement.dataset.index;

    const selected = getSelected();
    if (!selected) return;
 
    const rowBorderColorArray = JSON.parse(
      selected?.dataset?.row_borderColors ?? "[]"
    );
   
    if (rowBorderColorArray.length == 0){
     
      return;
    }
    rowBorderColorArray.splice(parseInt(index), 1);
    selected.dataset.row_borderColors = JSON.stringify(
      rowBorderColorArray
    );
