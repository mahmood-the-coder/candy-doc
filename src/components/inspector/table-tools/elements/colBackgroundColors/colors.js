...
export const colBackgroundColors = document.createElement("div");
colBackgroundColors.classList.add("candyDoc__inspectorTableColBackgroundColors");
const colorItem = document.createElement("div");
colorItem.classList.add(
  "candyDoc__inspectorTableColColorItem",
...
addIcon.addEventListener("mousedown", () => {
  ...
  const colors = JSON.parse(
    selected?.dataset?.col_backgroundColors ?? "[]"
  );
  colors.push("#ffffff");
  selected.dataset.col_backgroundColors=JSON.stringify(colors)
  addTableColsBackgroundColors(colors)
  applyColColors(selected)
});
export function addTableColsBackgroundColors(colorsArray) {
  colBackgroundColors.innerHTML = "";
  colBackgroundColors.append(colorsLabel,addIcon);
  ...
  for (let index = 0; index < colorsArray.length; index++) {
    ...
    colBackgroundColors.append(colorItemClone);
    const color = colorsArray[index];
    colorItemClone.dataset.col_backgroundColor = color;
    ...
    input.addEventListener("input", (e) => {
      const selected = getSelected();
      if (!selected) return;
      const colBackgroundColors = JSON.parse(
        selected?.dataset?.col_backgroundColors ?? "[]"
      );
      colBackgroundColors[input.parentElement.dataset.index] =
        e.target.value;
      selected.dataset.col_backgroundColors =
        JSON.stringify(colBackgroundColors);
      input.style.color = e.target.value;
      input.style.backgroundColor = e.target.value;
      input.parentElement.dataset.col_backgroundColor = e.target.value;
      applyColColors(selected);
    });
...
window.addEventListener("mousedown",(e)=>{
    if(!e.target.classList.contains("candyDoc__inspectorTableColColorItemRemoveIcon"))return;
    const index = e.target.parentElement.dataset.index;

    const selected = getSelected();
    if (!selected) return;
 
    const colBackgroundColorArray = JSON.parse(
      selected?.dataset?.col_backgroundColors ?? "[]"
    );
   
    if (colBackgroundColorArray.length == 0){
     
      return;
    }
    colBackgroundColorArray.splice(parseInt(index), 1);
    selected.dataset.col_backgroundColors = JSON.stringify(
      colBackgroundColorArray
    );
