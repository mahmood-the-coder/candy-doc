...
export const rowBackgroundColors = document.createElement("div");
rowBackgroundColors.classList.add("candyDoc__inspectorTableBackgroundColors");
const colorItem = document.createElement("div");
colorItem.classList.add(
  "candyDoc__inspectorTableRowColorItem",
  "candyDoc__inspectorTableColorDragItem"
);
const colorItemController = getColorInput();
...
addIcon.addEventListener("mousedown", () => {
  ...
  const colors = JSON.parse(
    selected?.dataset?.row_backgroundColors ?? "[]"
  );
  colors.push("#ffffff");
  selected.dataset.row_backgroundColors=JSON.stringify(colors)
  addTableRowsBackgroundColors(colors)
  applyRowColors(selected)
});
export function addTableRowsBackgroundColors(colorsArray) {
  rowBackgroundColors.innerHTML = "";
  rowBackgroundColors.append(colorsLabel,addIcon);
  ...
  for (let index = 0; index < colorsArray.length; index++) {
    ...
    rowBackgroundColors.append(colorItemClone);
    const color = colorsArray[index];
    colorItemClone.dataset.row_backgroundColor = color;
    ...
    input.addEventListener("input", (e) => {
      const selected = getSelected();
      if (!selected) return;
      const rowBackgroundColors = JSON.parse(
        selected?.dataset?.row_backgroundColors ?? "[]"
      );
      rowBackgroundColors[input.parentElement.dataset.index] =
        e.target.value;
      selected.dataset.row_backgroundColors =
        JSON.stringify(rowBackgroundColors);
      input.style.color = e.target.value;
      input.style.backgroundColor = e.target.value;
      input.parentElement.dataset.row_backgroundColor = e.target.value;
      applyRowColors(selected);
    });
...
window.addEventListener("mousedown",(e)=>{
    if(!e.target.classList.contains("candyDoc__inspectorTableRowColorItemRemoveIcon"))return;
    const index = e.target.parentElement.dataset.index;

    const selected = getSelected();
    if (!selected) return;
 
    const rowBackgroundColorArray = JSON.parse(
      selected?.dataset?.row_backgroundColors ?? "[]"
    );
 
    if (rowBackgroundColorArray.length == 0){
     
      return;
    }
    rowBackgroundColorArray.splice(parseInt(index), 1);
    selected.dataset.row_backgroundColors = JSON.stringify(
      rowBackgroundColorArray
    );
