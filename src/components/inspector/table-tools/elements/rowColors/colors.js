...
export const rowTextColors = document.createElement("div");
rowTextColors.classList.add("candyDoc__inspectorTableRowTextColors");
const colorItem = document.createElement("div");
colorItem.classList.add(
  "candyDoc__inspectorTableRowTextColorItem",
...
addIcon.addEventListener("mousedown", () => {
  ...
  const colors = JSON.parse(
    selected?.dataset?.row_textColors ?? "[]"
  );
  colors.push("#ffffff");
  selected.dataset.row_textColors=JSON.stringify(colors)
  addTableRowsTextColors(colors)
  applyRowColors(selected)
});
export function addTableRowsTextColors(colorsArray) {
  rowTextColors.innerHTML = "";
  rowTextColors.append(colorsLabel,addIcon);
  ...
  for (let index = 0; index < colorsArray.length; index++) {
    ...
    rowTextColors.append(colorItemClone);
    const color = colorsArray[index];
    colorItemClone.dataset.row_textColor = color;
    ...
    input.addEventListener("input", (e) => {
      const selected = getSelected();
      if (!selected) return;
      const rowTextColors = JSON.parse(
        selected?.dataset?.row_textColors ?? "[]"
      );
      rowTextColors[input.parentElement.dataset.index] =
        e.target.value;
      selected.dataset.row_textColors =
        JSON.stringify(rowTextColors);
      input.style.color = e.target.value;
      input.style.backgroundColor = e.target.value;
      input.parentElement.dataset.row_textColor = e.target.value;
      applyRowColors(selected);
    });
...
window.addEventListener("mousedown",(e)=>{
    if(!e.target.classList.contains("candyDoc__inspectorTablerowTextColorItemRemoveIcon"))return;
    const index = e.target.parentElement.dataset.index;

    const selected = getSelected();
    if (!selected) return;
 
    const rowTextColorArray = JSON.parse(
      selected?.dataset?.row_textColors ?? "[]"
    );
   
    if (rowTextColorArray.length == 0){
     
      return;
    }
    rowTextColorArray.splice(parseInt(index), 1);
    selected.dataset.row_textColors = JSON.stringify(
      rowTextColorArray
    );
