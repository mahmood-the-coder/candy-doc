...
export const image = document.createElement("div");
image.classList.add("candyDoc__inspectorImageInput");
const imageController = document.createElement("div");
const label = document.createElement("div");
label.classList.add("candyDoc__inspectorImageInputLabel");
label.innerText = "Image Source";
image.append(label, imageController, slicer);
imageController.classList.add("candyDoc__icon");
...
imageController.addEventListener("mouseup", () => {
  openFileModal((image)=>{
     const dataURL=image.querySelector("img").src;
     slicer.querySelector(".candyDoc__inspectorSlicerImage").src=dataURL;
     const selected=getSelected();
     if(!selected)return;
     selected.querySelector(".target").style.borderImageSource=`url('${dataURL}')`
  })
