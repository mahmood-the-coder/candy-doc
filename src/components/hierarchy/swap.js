...
export function swap(clone, target, others) {
  ...
  others.forEach((o) => {
    ...
    if (isCollideY(otherRect, cloneRect)) {
      ...
      if (o.dataset?.type == "parent") {
        const targetItemIndex = getHierarchyItems().findIndex(
          (i) => i.id == target.dataset.id
        );
        if (getHierarchyItems()[targetItemIndex])
          getHierarchyItems()[targetItemIndex].parentId = null;
        ...
        if (
          ...
          if (getHierarchyItems()[targetItemIndex])
          {
            getHierarchyItems()[targetItemIndex].parentId = o.dataset.id;
