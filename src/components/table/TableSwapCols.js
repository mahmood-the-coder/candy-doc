...
export function initSwapCols() {
  ...
  zoom=window.devicePixelRatio *(page.getBoundingClientRect().width/page.offsetWidth);
    for (let index = 0; index < getTemplateCols(table).length; index++) {
      let handle = null;
      const exist = table.querySelector(`[data-swap-col='${index + 1}']`);
      if (exist) handle = exist;
      else handle = createColSwapHandle();

      handle.dataset.swapCol = (index + 1).toString();
    ...
    isDragging = true;
    toSwap = [
      ...table.querySelectorAll(`[data-col='${e.target.dataset.swapCol}']`),
    ]
    ...
    deltaX = 0;
    if (toSwap.length >= 1) startX = toSwap[0].offsetLeft;

    table.querySelectorAll(".candyDoc__tableColSwapHandle").forEach((h) => {
      h.style.opacity = "0";
    });
    ...
    clones.forEach((c, index) => {
      table.append(c);
      c.style.top = toSwap[index].offsetTop + "px";
      c.style.left = toSwap[0].offsetLeft + "px";
      c.style.scale = "0.85";
      c.style.opacity = "0.5";
...
function createColSwapHandle() {
  const handle = document.createElement("div");
  handle.id = "candy-doc-col-swap-handle";
  handle.classList.add("candyDoc__tableColSwapHandle", "candyDoc__tableHandle","candyDoc__icon");
